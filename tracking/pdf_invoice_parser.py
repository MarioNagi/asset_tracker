...
class PDFInvoiceParser:
    ...
    def _extract_service_provider(self, text: str) -> str:
        ...
        # Enhanced lookup in first few lines for new format invoices
        for i, line in enumerate(lines[:10]):  # Check first 10 lines
            ...
            # Look for lines that contain business indicators
            if (line and 
                len(line) > 5 and 
                any(word in line.upper() for word in ['PTY', 'LTD', 'TYRES', 'MOTORS', 'AUTOMOTIVE', 'SERVICES']) and
                not any(skip in line.upper() for skip in ['TAX INVOICE', 'BILL TO', 'ADDRESS', 'PHONE', 'INVOICE NUMBER'])):
                return line.title()
